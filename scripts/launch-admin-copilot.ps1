...
if ($isAdmin) {
    ...
    Write-Host "Not running as Administrator - some operations may be limited" -ForegroundColor Yellow
}
    ...
    if ($?) {
        ...
        try {
            ...
            if ($?) {
                ...
                Write-Host "GitHub Copilot CLI extension not found" -ForegroundColor Yellow
                Write-Host "   Install with: gh extension install github/gh-copilot" -ForegroundColor Cyan
            }
        } catch {
            Write-Host "GitHub Copilot CLI extension not available" -ForegroundColor Yellow
